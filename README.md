...
#### การตั้งค่าสภาพแวดล้อมสำหรับเครื่องมือ (Tooling Setup)

เครื่องมือบางอย่างในโฟลเดอร์ `/tools` (เช่น `check-duplicates.js`) จำเป็นต้องใช้ไลบรารีภายนอกจาก Node.js ในการทำงาน ก่อนที่จะรันสคริปต์เหล่านี้เป็นครั้งแรก คุณต้องตั้งค่าสภาพแวดล้อมก่อน:

1.  **ตรวจสอบว่าคุณมี Node.js และ npm ติดตั้งอยู่**

2.  **สร้างไฟล์ `package.json`:** หากยังไม่มีไฟล์นี้ในโปรเจกต์ ให้เปิด Terminal แล้วรันคำสั่ง:
    ```bash
    npm init -y
    ```

3.  **ติดตั้ง Dependencies:** รันคำสั่งต่อไปนี้เพื่อติดตั้งไลบรารีที่จำเป็น (เช่น `chalk` สำหรับสีใน Terminal):
    ```bash
    npm install chalk
    ```

หลังจากติดตั้งเสร็จแล้ว คุณจะสามารถรันสคริปต์ต่างๆ ในโฟลเดอร์ `/tools` ได้ตามปกติ เช่น `node tools/check-duplicates.js`
